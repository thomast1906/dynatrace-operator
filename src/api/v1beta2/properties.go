...
func (dk *DynaKube) ActiveGateMode() bool {
	return len(dk.Spec.ActiveGates) > 0
}

func (dk *DynaKube) ActiveGateByName(name string) *ActiveGateSpec {
	for _, activeGate := range dk.Spec.ActiveGates {
		if activeGate.Name == name {
			return &activeGate
		}
	}
	return nil
...
func (dk *DynaKube) IsActiveGateMode(mode CapabilityDisplayName) bool {
	for _, activeGate := range dk.Spec.ActiveGates {
		for capabilityName := range activeGate.Capabilities {
			if capabilityName == mode {
				return true
			}
		}
	}
