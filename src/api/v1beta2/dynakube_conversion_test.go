...
func TestConversion_ConvertFrom(t *testing.T) {
	...
	/*
		...
		compareAlphaCapability(t,
			...
			convertedDynakube.Spec.Routing.ActiveGateProperties)
		...
		compareAlphaCapability(t,
			...
			convertedDynakube.Spec.KubernetesMonitoring.ActiveGateProperties)
	*/
	assert.Len(t, convertedDynakube.Spec.ActiveGates, 0)
...
/*func compareAlphaCapability(t *testing.T, expectedCapability v1alpha1.CapabilityProperties, actualCapability ActiveGateProperties) {
	...
	oldDynakube := &DynaKube{
		...
		Spec: DynaKubeSpec{
			...
			/*
				Routing: RoutingSpec{
					Enabled:              true,
					ActiveGateProperties: prepareBetaCapability(),
				},
...
/*func compareBetaCapability(t *testing.T, expectedCapability ActiveGateProperties, actualCapability v1alpha1.CapabilityProperties) {
	assert.Equal(t, expectedCapability.Replicas, actualCapability.Replicas)
	assert.Equal(t, expectedCapability.Group, actualCapability.Group)
	assert.Equal(t, expectedCapability.CustomProperties.ValueFrom, actualCapability.CustomProperties.ValueFrom)
	assert.Equal(t, expectedCapability.CustomProperties.Value, actualCapability.CustomProperties.Value)
...
func prepareBetaCapability() ActiveGateProperties {
	intVal := int32(3)
	return ActiveGateProperties{
		Replicas: &intVal,
		Group:    "test-activegate-group",
